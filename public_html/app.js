...
analyseBtn.addEventListener("click", async function () {
  ...
  try {
    ...
    const response = await fetch("/api/analyze", {
      ...
      body: JSON.stringify({
        image: currentImage                     // the base64 image string
      })
    });
    ...
    // ----- Read the JSON the server sent back -----
...
simplerBtn.addEventListener("click", async function () {
  ...
  try {
    ...
    // ----- Send the current explanation to /api/simplify -----
    ...
    const p = document.createElement("p");
    p.className   = "result-text";
    p.textContent = data.simplified;
    explanationBox.appendChild(p);

    // Update lastExplanation so Read Aloud reads the new version
    lastExplanation = data.simplified;
